...
use po_k_proto::{
    BatchHeader, BatchKind, HeartbeatRow, IngestResponse, SubagentMetaRow, HEADER_API_KEY,
...
impl Shipper {
    ...
    pub async fn ship_heartbeat(&self, rows: &[HeartbeatRow]) -> Result<()> {
        if rows.is_empty() {
            return Ok(());
        }
        ...
        let url = format!("{}/ingest/heartbeat", self.server_url.trim_end_matches('/'));
        let resp = self
            .client
            .post(&url)
            .header(HEADER_API_KEY, &self.api_key)
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("heartbeat ingest failed: {status}: {body}");
        }
        Ok(())
    }
}
