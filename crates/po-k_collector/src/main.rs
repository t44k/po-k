...
async fn main() -> Result<()> {
    ...
    let stats = scan_and_ship_all(&root, &store, &shipper, &machine_id).await?;
    info!(
    ...
    run_live(root, store, shipper, machine_id).await
}
...
async fn scan_and_ship_all(
    ...
    for path in files {
        match scan_file(root, &path, store, machine_id).await {
            Ok(Some(result)) => {
...
async fn run_live(
    ...
    loop {
        ...
        match scan_and_ship_all(&root, &store, &shipper, &machine_id).await {
